...
postsRouter.post("/posts", requireUser, async (req, res) => {
    try {
...
postsRouter.post("/posts/comment", requireUser, async (req, res) => {
    try {
        const body = requireObject(req.body);
...
postsRouter.post("/posts/like", requireUser, async (req, res) => {
    try {
        const body = requireObject(req.body);
        const postId = requireString(body, "postId", { trim: true, min: 8 });
